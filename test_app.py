...
def dummy_cluster_annotations():
    filepath = "Cluster annotation.xlsx"
...
def test_parse_cluster_annotations():
    mapping = parse_cluster_annotations("Cluster annotation.xlsx")
...
def test_streamlit_app_and_resolution(tmp_path):
    ...
    dummy_csv = "combined_expression.csv"
    ...
    try:
        ...
        # Element 1 in radio is Gene Selection Mode
        at.radio[1].set_value("Paste Bulk Gene Set").run()
        assert not at.exception
        ...
        at.radio[1].set_value("Predefined Gene Groups").run()
        assert not at.exception
        ...
        at.radio[1].set_value("Generate Group from Cell Type").run()
        assert not at.exception
        ...
        # st.radio("Criterion:", ...) is the 3rd radio (index 2)
        assert len(at.radio) > 2
        at.radio[2].set_value("Expression Threshold").run()
...
def test_coexpression_logic():
    from app import calculate_ksg_mi, compute_coexpression_groups
    import numpy as np
    ...
    results_ksg = compute_coexpression_groups('ab', df, metric="KSG Mutual Information")
    assert len(results_ksg) > 0
    assert 'ksg_mi' in results_ksg[0]
